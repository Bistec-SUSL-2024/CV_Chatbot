...
def extract_sections(text):
    section_patterns = {
        "Personal Details": r"(?i)(contact details|personal information|email|phone|address):?",
        "Skills": r"(?i)(skills|technical skills|competencies):?",
        "Work Experience": r"(?i)(work experience|employment history|professional experience):?",
        "Education": r"(?i)(education|certifications|licenses|university|achievementsqualifications|academic background|school):?",
...
# Function to chunk text: One chunk per section
def chunk_text_by_section(sections):
    chunks = []
    for section_name, section_content in sections.items():
        chunks.append({
            "id": f"{section_name}_chunk_0",
            "section": section_name,
            "content": section_content.strip()  # Ensure content is cleaned and stripped of extra whitespace
        })
    return chunks
...
if not markdown_files:
    ...
    for md_file in markdown_files:
        ...
        try:
            ...
            # Show extracted chunks in the terminal
            for chunk in chunks:
                print(f"\nChunk ID: {chunk['id']}")

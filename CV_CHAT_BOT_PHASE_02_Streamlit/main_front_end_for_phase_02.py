...
if st.session_state.cv_results:
    st.markdown("<div class='candidates-section'><h6>Ranked Candidates:</h6></div>", unsafe_allow_html=True)
    for idx, result in enumerate(st.session_state.cv_results, start=1):
        ...
        with col2:
            if st.button(f"Show CV", key=f"show_cv_{result['cv_id']}"):
                try:
                    response = requests.post(
                        "http://localhost:8000/show_cv",  
                        json={"cv_id": result['cv_id']}
                    )
                    if response.status_code == 200:
                        # st.success(f"Successfully opened CV for **{result['cv_id']}**")
                        pass
                    else:
                        st.error(f"Failed to show CV for {result['cv_id']}")
                except Exception as e:
                    st.error(f"Error showing CV for {result['cv_id']}: {e}")
        with col3:
...
if st.session_state.show_chat:
    with st.sidebar:
        ...
        if ask_button and prompt:
            chatbot_response = get_backend_response(prompt)  
